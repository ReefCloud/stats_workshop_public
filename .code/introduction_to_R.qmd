...
ls()           #confirm that all objects have been removed
```
...
```{r, mhidden=TRUE}
#| label: code 21
cat(paste(apropos("^mean", ignore=FALSE),c("\t","\t","\t","\n","\t","\t","\n")))
```
...
sampleDates <- seq(from=as.Date("2000-02-29"),to=as.Date("2000-04-30"), by="7 days")
...
mean(seq(from = as.Date("2000-02-29"),
  to = as.Date("2000-04-30"),
  by = "7 days"))
```
...
```{r, mhidden=TRUE}
#| label: code 62
EXPERIMENT[c('TEMPERATURE','SHADE')]
```
...
#| label: code 63
EXPERIMENT[['TEMPERATURE']]
#OR equivalently (recall earlier)
EXPERIMENT$TEMPERATURE
EXPERIMENT[c('TEMPERATURE','SHADE')]
```
...
#OR as a data frame (on the items that are of equal size)
```
...
```{r, mhidden=TRUE}
#| label: code 81
stats::filter(x = 1:100, filter = rep(1, 3))
```
